<template>
  <el-breadcrumb class="app-breadcrumb" separator="//" style="height:50px;backgroundColor:rgba(255, 255, 255, 1);borderRadius:4px 4px 10px 10px;padding:0px 20px 0px 20px;boxShadow:0px 0px 0px #f903d4;borderWidth:1px 1px 7px 0;borderStyle:solid;borderColor:rgba(251, 213, 1, 1);">
    <transition-group name="breadcrumb" class="box" :style="1==1?'justifyContent:flex-start;':1==2?'justifyContent:center;':'justifyContent:flex-end;'">
      <el-breadcrumb-item v-for="(item,index) in levelList" :key="item.path">
        <span v-if="item.redirect==='noRedirect'||index==levelList.length-1" class="no-redirect" style="color:rgba(120, 120, 120, 1)">{{ item.name }}</span>
    ...
    breadcrumbStyleChange(val) {
      this.$nextTick(()=>{
        document.querySelectorAll('.app-breadcrumb .el-breadcrumb__separator').forEach(el=>{
          el.innerText = "//"
          el.style.color = "rgba(54, 54, 54, 1)"
        })
        ...
        document.querySelectorAll('.app-breadcrumb .el-breadcrumb__inner .no-redirect').forEach(el=>{
          el.style.color = "rgba(120, 120, 120, 1)"
        })
